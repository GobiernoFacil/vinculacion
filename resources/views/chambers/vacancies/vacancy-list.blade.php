...
<div class="row">
  ...
  <div class="col-sm-2 col-sm-offset-10">
    <p><a href="{{url('tablero-camara/vacante/crear')}}" class="btn add"> + Crear vacante</a></p>
  </div>
  <div class="col-sm-12">
    <div>
      @if($vacancies->count())
      <ul class="list">
        ...
        <li class="clearfix">
          ...
          <span class="col-sm-3 col-xs-5">
          	<a href="{{url("tablero-camara/vacante/ver/{$vacancy->company['id']}/{$vacancy->id}")}}" class="link_view"> {{$vacancy->job}}</a>
          	<br>
          ...
          <span class="col-sm-1 col-xs-2">{!! $vacancy->applicants()->count() > 0 ? '<a href="'. url('tablero-camara/vacante/'. $vacancy->id). '#applicants">' . $vacancy->applicants()->count(). '</a>' : $vacancy->applicants()->count() !!}</span>
          <span class="col-sm-1 nomobile">{{$vacancy->interviews()->count()}}</span>
          <span class="col-sm-3 col-xs-12 right">
            <a href="{{url("tablero-camara/vacante/ver/{$vacancy->company['id']}/{$vacancy->id}")}}" class="btn add xs">Ver</a>
            <a href="{{url("tablero-camara/vacante/editar/{$vacancy->company['id']}/{$vacancy->id}")}}" class="btn xs">Editar</a>
          	<a href="{{url("tablero-camara/vacante/habilitar/{$vacancy->company['id']}/{$vacancy->id}")}}" class="btn add xs">{{$vacancy->status == 1 ? "Ocultar" : "Publicar" }}</a>
            <a data-job="{{$vacancy->job}}" href="{{url("tablero-camara/vacante/eliminar/{$vacancy->company['id']}/{$vacancy->id}")}}" class="btn danger xs">
              Eliminar
            </a>
          </span>
        </li>
